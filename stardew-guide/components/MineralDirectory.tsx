...
type MuseumFilter = "All" | "Museum item" | "Not for museum";

const museumFilters: MuseumFilter[] = ["All", "Museum item", "Not for museum"];

export function MineralDirectory({ minerals }: { minerals: Mineral[] }) {
  const [query, setQuery] = useState("");
  const [museumFilter, setMuseumFilter] = useState<MuseumFilter>("All");
  const [location, setLocation] = useState("All");

  const locationOptions = useMemo(() => ["All", ...Array.from(new Set(minerals.flatMap((item) => item.locations))).sort()], [minerals]);

  const filteredMinerals = useMemo(() => {
    const normalizedQuery = query.trim().toLowerCase();

    return minerals.filter((item) => {
      const matchesQuery =
        item.name.toLowerCase().includes(normalizedQuery) ||
        item.locations.some((entry) => entry.toLowerCase().includes(normalizedQuery)) ||
        item.uses.some((entry) => entry.toLowerCase().includes(normalizedQuery));

      const matchesMuseum =
        museumFilter === "All" ||
        (museumFilter === "Museum item" && item.museumDonation) ||
        (museumFilter === "Not for museum" && !item.museumDonation);

      const matchesLocation = location === "All" || item.locations.includes(location);

      return matchesQuery && matchesMuseum && matchesLocation;
    });
  }, [location, minerals, museumFilter, query]);
    ...
    <section className="space-y-4">
      <div className="codex-list-shell rounded-md p-3 sm:p-4">
        <div className="mb-3 flex flex-col gap-2 border-b border-[rgba(122,86,56,0.16)] pb-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-pond">Mineral Index</p>
            <h2 className="mt-1 text-lg font-black text-green-950">Browse minerals</h2>
            <p className="mt-1 text-xs font-semibold text-green-950/55">{minerals.length} minerals indexed</p>
          </div>
          <p className="w-fit rounded-sm border border-pond/20 bg-pond/10 px-2.5 py-1 text-xs font-black uppercase tracking-[0.12em] text-pond">
            {filteredMinerals.length} results
          </p>
        </div>
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_12rem_14rem] lg:items-end">
          <label className="block min-w-0">
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-green-950/50">Search minerals</span>
            <input
              ...
              placeholder="Quartz, Geodes..."
              type="search"
              value={query}
            />
          </label>

          <FilterSelect label="Museum" onChange={setMuseumFilter} options={museumFilters} value={museumFilter} />
            ...
            >
              <div className="relative flex h-full min-w-0 flex-col gap-4 px-5 py-5 pl-7">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <Fact label="Museum donation" value={item.museumDonation ? "Yes" : "No"} tone={item.museumDonation ? "default" : "muted"} />
                </div>
