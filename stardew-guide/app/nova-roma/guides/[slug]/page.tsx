...
import { getAllNovaRomaGuideArticles, getAllNovaRomaGuideSlugs, getNovaRomaGuideArticleBySlug } from "@/lib/nova-roma/data";
import type { NovaRomaGuideArticle, NovaRomaGuideArticleBlock } from "@/lib/nova-roma/types";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!article) {
    return {
      title: "Nova Roma Guide Not Found | Player Codex",
      description: "This Nova Roma guide could not be found."
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `https://playercodex.app/nova-roma/guides/${article.slug}`
    },
    openGraph: {
      title: `${article.title} | Player Codex`,
      description: article.summary,
      url: `https://playercodex.app/nova-roma/guides/${article.slug}`,
...
export function generateStaticParams() {
  return getAllNovaRomaGuideSlugs().map((slug) => ({ slug }));
}

export default async function NovaRomaGuideDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getNovaRomaGuideArticleBySlug(slug);
  ...
  const allArticles = getAllNovaRomaGuideArticles();
  ...
  const headings = article.blocks.filter((block): block is Extract<NovaRomaGuideArticleBlock, { type: "heading" }> => block.type === "heading" && block.level === 2);

  return (
    <PageShell eyebrow="Nova Roma Guide" kicker={article.summary} title={article.title}>
      <article className="space-y-5">
        <nav aria-label="Breadcrumb" className="rounded-md border border-green-950/10 bg-white/70 px-4 py-3 text-sm font-black text-green-950/58 sm:px-5">
          <ol className="flex flex-wrap items-center gap-2">
            ...
            <li>
              <Link className="hover:text-green-950" href="/nova-roma">
                Nova Roma
              </Link>
            </li>
            <li aria-hidden="true">/</li>
            <li>
              <Link className="hover:text-green-950" href="/nova-roma/guides">
                Guides
              </Link>
            </li>
          </ol>
        ...
        <div>
          <Link className="inline-flex w-fit rounded-sm border border-green-950/14 bg-green-950/[0.04] px-3 py-2 text-sm font-black text-green-950 transition hover:bg-green-950/[0.08]" href="/nova-roma/guides">
...
function ArticleBlock({ block, index }: { block: NovaRomaGuideArticleBlock; index: number }) {
...
function NavArticle({ article, label }: { article: NovaRomaGuideArticle; label: string }) {
  return (
    <Link className="group rounded-md border border-green-950/10 bg-white/80 px-4 py-4 transition hover:-translate-y-0.5 hover:border-green-950/20 hover:bg-white" href={`/nova-roma/guides/${article.slug}`}>
